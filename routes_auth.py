...
from models_auth import User, Otp  # Asegúrate de tener estos modelos
# User: id, phone, email, role, name, password_hash (añádelo si no existe)
# Otp:  id, created_at, target, code, expires_at, tries, used, meta
...
PASSLINK_TTL_MIN = int(os.getenv("PASSLINK_TTL_MIN", "15"))  # Validez del enlace de contraseña
...
def me():
    # Este endpoint se puede ampliar para leer Authorization: Bearer y devolver el usuario
    return jsonify(ok=True, message="Attach a JWT parser here if needed")

@bp_auth.post("/api/auth/create_user")
def create_user():
    # Protegido por admin key
...
# ----- OTP por SMS (para primera vez / alternativa) -----
...
def request_otp():
    ...
    otp = Otp.new(target, ttl_sec=300)  # genera y fija expires_at
    db.session.add(otp); db.session.commit()
    # Aquí enviarías SMS/email real. Por seguridad NO devolvemos el code.
...
# ----- Enlace para crear/recuperar contraseña (SMS link) -----
...
def request_password_link():
    ...
    # Aquí envías el enlace por SMS con tu proveedor
    current_app.logger.info("[AUTH] passlink for %s -> %s", phone, link)
...
def set_password():
    ...
    # Asegúrate de tener el campo password_hash en User
