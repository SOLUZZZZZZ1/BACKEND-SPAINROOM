...
def create_contract():
    """
    Crea contrato (draft) y sus líneas (items) con sub_ref SR-XXXXX-01..N
    body:
      { owner_id, tenant_id, franchisee_id?, rooms:[{id}], splits? {owner,franq}, meta_json? }
    """
    ...
    c = Contract(ref=ref, owner_id=owner_id, tenant_id=tenant_id, franchisee_id=franchisee_id, status="draft",
...
def mark_signed():
    """ Marca contrato como firmado; habilita subida de fotos/fichas por franquiciado. """
    data = request.get_json(force=True)
...
def get_contract(ref):
    ...
    return jsonify(ok=True, contract={
        "ref": c.ref, "status": c.status, "owner_id": c.owner_id, "tenant_id": c.tenant_id, "franchisee_id": c.franchisee_id,
        "items": [
