...
# SpainRoom — Voice Backend (ConversationRelay) — ES STABLE (TwiML Escaped, FIX)
# FastAPI app para Twilio Voice usando <ConversationRelay> (STT+TTS por Twilio)
# - Español solo. Espera 'setup' antes de hablar.
# - TwiML con atributos ESCAPADOS y strings correctamente citados.
...
# Ejecutar: uvicorn codigo_flask_es_stable_escaped_fix:app --host 0.0.0.0 --port $PORT --proxy-headers
...
app = FastAPI(title="SpainRoom Voice — ConversationRelay ES Stable (Escaped, FIX)")
...
async def root(request: Request):
    ...
    html = (
        "<h2>SpainRoom Voice — ConversationRelay (ES, Escaped TwiML FIX)</h2>\n"
...
# SpainRoom — Voice Backend (ConversationRelay) — ES STABLE (TwiML Escaped)
# FastAPI app para Twilio Voice usando <ConversationRelay> (STT+TTS por Twilio)
# - Español solo. Espera 'setup' antes de hablar.
# - TwiML con atributos ESCAPADOS (evita errores de XML por ENV con comillas o símbolos).
# - FSM 5 campos (rol, población, zona, nombre, teléfono) + respuestas de información.
# - Endpoints: /voice/answer_cr · /voice/fallback · WS /cr · /assign · /health · /diag_runtime
...
async def answer_cr(request: Request):
    ...
    # Construir atributos con quoteattr (escapa &, ", <, > y rodea con comillas)
    attrs = [
        f"url={quoteattr(ws_url)}",
        f"language={quoteattr(lang)}",
        f"transcriptionLanguage={quoteattr(trans_lang)}",
