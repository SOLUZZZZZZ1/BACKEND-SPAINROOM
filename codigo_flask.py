...
# - IVR /voice/*: formulario conversacional por turnos (rol→población→nombre→teléfono→nota→confirmación)
# - Voz natural: un solo <Say> por turno (SSML <prosody> + micro-pausas), barge-in, re-prompts amables
# - Tareas con transcript + SMS a franquiciados (multi-destino)
# - Sistema de TERRITORIOS (España): microzonas (bbox) y tiles de rejilla; persistencia JSON; endpoints /territories/*
# - Admin: UI de tareas /admin/tasks y actualización /tasks/update
# - Geocoder de ejemplo (Nominatim) y Jobs mock
...
def _ssml(text: str) -> str:
    """Marcas: [[b300]] -> <break time="300ms">, [[digits:600123123]] -> <say-as ...>"""
...
# ==============================
# TERRITORIOS (España): microzonas y tiles
# ==============================
...
# === Endpoints de territorios (admin) ===
def _auth_terr():
    if not TERR_TOKEN: return True  # sin token → abierto (puedes exigirlo poniendo valor)
...
def terr_claim():
    ...
    if mode=="tile":
        ...
        TERR["tiles"][key]={"label":label,"phones":phones}
        _save_territories(TERR); return jsonify(ok=True,key=key),200
    elif mode=="bbox":
        city=_slug_city(payload.get("city",""))
        bbox=payload.get("bbox")  # [minlat,minlng,maxlat,maxlng]
...
def _parse_city(s:str)->str:
    ...
    # si no es provincia conocida, igualmente devolvemos slug de ciudad para geocode
...
def voice_confirm_summary():
    ...
    lat=lng=None
    city_query = mem["zone"]
    if city_query and city_query not in PROVS:  # si capturó ciudad no-provincia
        lt,lg=_geocode_city(city_query); lat,lng=lt,lg
    if (lat is None or lng is None) and city_query in PROVS:  # si es provincia, aproximar al centro provincial (geocode)
...
<style>
:root{--card:#121923;--bg:#0c141d;--muted:#9fb0c3;--txt:#e7eef7;--btn:#2563eb}
...
# ==============================
# Territorios: Admin token helper
# ==============================
def _assert_terr_admin():
