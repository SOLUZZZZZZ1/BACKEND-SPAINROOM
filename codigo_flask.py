...
# codigo_flask_bargein24k_post.py — SpainRoom Voice (Twilio 8 kHz ↔ OpenAI 24 kHz) con barge‑in
# - 24 kHz end‑to‑end (tono correcto)
# - Barge‑in real (se puede interrumpir; cancela respuesta)
# - Micro‑pausas y límite de frase
#
...
#   CHUNK_MS=20, PACE_MS=20, HWM_FRAMES=50, BURST_MAX=8, PREROLL_MS=400
#   PAUSE_EVERY_MS=1200, BARGE_VAD_DB=-30, BARGE_RELEASE_MS=500, BARGE_SEND_SILENCE=1, MAX_UTTER_MS=5500
#   SYSTEM_PROMPT="Eres Nora..."
#   STRIPE_SECRET_KEY=sk_live_xxx (solo diagnóstico)
...
async def twilio_stream(ws_twilio: WebSocket):
    ...
    async def twilio_sender():
        ...
        while True:
            # Si el cliente habla, no reproducimos la IA y mandamos silencio para mantener reloj
            if barge_active:
                try:
                    while True:
    ...
    async def connect_ai_after_start():
        ...
        try:
            ...
            # Sesión: PCM16 24k + VAD del lado servidor
            ...
            try:
                async for raw in ws_ai:
                    ...
                    if t in ("response.audio.delta", "response.output_audio.delta"):
                        if barge_active:
                            continue  # ignorar audio mientras el cliente habla
        ...
        try:
            while True:
                ...
                if ev == "start":
                    ...
                    # VAD simple: detecta voz del cliente
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("codigo_flask_bargein24k_post:app", host="0.0.0.0", port=8000)
