# routes_wa.py — WhatsApp Business API (MessageBird / Meta / 360dialog)
...
WA_API_KEY      = os.getenv("WA_API_KEY", "").strip()                   # LIVE-xxxx...
WA_ENDPOINT     = os.getenv("WA_ENDPOINT", "https://conversations.messagebird.com/v1/send").strip()
WA_CHANNEL_ID   = os.getenv("WA_CHANNEL_ID", "").strip()                # Channel ID del conector WA
WA_SIGNING_KEY  = os.getenv("WA_SIGNING_KEY", "").strip()               # opcional: firma HMAC webhooks
...
def _send_template_360dialog(to: str, template: str, params: list, lang="es"):
    ...
    r = requests.post(WA_ENDPOINT, headers=headers, json=body, timeout=15)
    return r.status_code, r.text
...
def webhooks_wa():
    # Verificación Meta (si algún día usas Cloud API)
    ...
    _log("[WA WEBHOOK] provider=%s payload=%s", WA_PROVIDER, json.dumps(data)[:500])
