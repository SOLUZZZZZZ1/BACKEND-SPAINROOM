...
# normaliza tipos
df["provincia"] = df["provincia"].astype(str).strip()
df["municipio"] = df["municipio"].astype(str).str.strip()
df["poblacion"] = pd.to_numeric(df["poblacion"], errors="coerce").fillna(0).astype(int)

# 1) fuera provincias "pobmun..." y vacías
df = df[(df["provincia"] != "") & (~df["provincia"].str.lower().str.startswith("pobmun"))]

# 2) fuera municipios que son solo códigos (ej. 0010, 0013A)
df = df[~df["municipio"].str.fullmatch(r"\d+[A-Za-z]?", na=False)]

# 3) fuera entidades no-municipio (distritos, secciones, parroquias, pedanías, etc.)
bad_tokens = [
    "distrit", "sección", "seccion", "barrio", "entidad", "parroquia",
    "pedanía", "pedania", "aldea", "núcleo", "nucleo", "caserío", "caserio", "paraje"
]
...
MAD = {
    "centro","arganzuela","retiro","salamanca","chamartín","chamartin","tetúan","tetuan",
    "chamberí","chamberi","fuencarral-el pardo","moncloa-aravaca","latina","carabanchel","usera",
    "puente de vallecas","moratalaz","ciudad lineal","hortaleza","villaverde","villa de vallecas",
    "vicálvaro","vicalvaro","san blás-canillejas","san blas-canillejas","barajas"
}
BCN = {
    "ciutat vella","eixample","sants-montjuïc","sants-montjuic","les corts","sarrià-sant gervasi","sarria-sant gervasi",
    "gràcia","gracia","horta-guinardó","horta-guinardo","nou barris","sant andreu","sant martí","sant marti"
}
...
# 5) dedup por (provincia, municipio): nos quedamos con la mayor población
